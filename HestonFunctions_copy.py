...
def PIntegrand(u, kappa, eta, sigma, rho, v0, r, tau, S0, K, j):
    F = S0 * np.exp(r * tau)
    x = np.log(F / K)
    a = kappa * eta
    ...
    C = kappa * (rminus * tau - (2 / (sigma**2)) * np.log((1 - g * np.exp(-d * tau)) / (1 - g)))

    top = np.exp(C * eta + D * v0 + 1j * u * x)
    bottom = 1j * u
    real_part = (top / bottom).real
    return real_part


def P(kappa, eta, sigma, rho, v0, r, tau, S0, K, j):
    value, _ = quad(PIntegrand, 0, np.inf, args=(kappa, eta, sigma, rho, v0, r, tau, S0, K, j))
    prob = 0.5 + (1/np.pi) * value
    return prob

def HestonCallClosedForm(kappa, eta, sigma, rho, v0, r, tau, S0, K):

    A = S0 * P(kappa, eta, sigma, rho, v0, r, tau, S0, K, 1)
    B = K * np.exp(-r * tau) * P(kappa, eta, sigma, rho, v0, r, tau, S0, K, 0)
